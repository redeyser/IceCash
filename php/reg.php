...
if ((isset($_POST['viz']))&&($_POST['viz'])){
   ...
   $cIce->change_trsc('count',$count);
    ...
    if ($cIce->find_price($_POST['code'])){
        $cIce->add_sale($_POST['code'],1,0);
...
<div class="info"> <? echo "[".$cIce->ice_status."] [".$type_check."] [".$cIce->ice_number."] {в кассе: ".$zsum[0]."/".$zsum[1]."}"; ?> </div>

<form name="shk" id="shk" method="post" action=reg.php#code> 
    <input id="code" name="code" class="big" type="text" value="" autocomlete="off"  onKeyDown="keyr(event);">
